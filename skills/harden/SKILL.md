...
- Clone with `git -c core.hooksPath=/dev/null clone --no-recurse-submodules <url>`, never a bare `git clone`. Hooks then cannot run out of the repository being fetched, and no `clone.recurseSubmodules` in any config level can pull in submodule content that was not asked for. Review `.gitmodules` first, run `git submodule update --init` only after the submodule sources check out, and keep git patched — recursive clone executes hooks at clone time (CVE-2024-32002, CVE-2025-48384).
