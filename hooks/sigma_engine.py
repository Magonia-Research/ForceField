...
ForceField's zero-false-positive-deny guarantee. The tiered config may downgrade
...
from hook_logging import clamp_and_emit, emit, log_guard_ran  # noqa: E402
from patterns import MAX_STDIN_BYTES  # noqa: E402

# Compiled rules live outside the plugin, under ForceField's own state directory.
# The plugin directory is a *cache*: every reinstall (and every version-bump
# update) deletes and re-copies it, which silently took the rules and the
# compiler venv with it -- and sigma_update.sh then exits 0 on the missing venv
# without saying anything, so 106 detections went quiet with no signal at all.
#
# It is also the safer home. filesystem_guard's Bash sink check covers
# ~/.claude/forcefield/ but never covered the plugin root, and sigma_update.sh
# executes the venv python at every SessionStart: a shell write there was
# unguarded code execution. Both artifacts are now behind that prompt.
SIGMA_DIR = Path.home() / ".claude" / "forcefield" / "sigma"
RULES_PATH = SIGMA_DIR / "rules.json"

# Ceiling on the compiled ruleset. The live file measured 177,650 bytes for
# 3,184 rules, so 16 MiB is ~94x the observed size and still a bound: a read
# with no ceiling is a memory exhaustion primitive on a path this account can
# replace.
...
def load_rules():
    ...
    Read through ``read_regular_text`` rather than ``open``: the ruleset sits at
    a path this account can replace, and a plain read-mode open of a FIFO blocks
    until a writer appears. Measured with ``mkfifo`` on this path: this hook went
    from 0.12 s to being killed at its 5 s timeout, delivering nothing.

    An unreadable or oversized ruleset yields no rules, which is the same state
    as "install.sh was never run" — the one the engine is already written to
    treat as silence.
    """
    raw = read_regular_text(RULES_PATH, MAX_RULES_BYTES)
    if not raw:
        return []
    try:
        data = json.loads(raw)
...
def match_field_value(field_value, modifier, values, all_required):
    ...
    # An empty match value is a universal matcher: "" is a substring of every
    # string, a prefix of every string and a suffix of every string, so a single
    # empty entry reaching `contains`/`startswith`/`endswith` turns its rule into
    # one that fires on everything. The compiler can emit one from a YAML key
    # with no value, and the rules are third-party. An empty value never
    # matches; if that leaves nothing to match on, the rule does not fire.
...
def get_field_value(field_name, command, image):
    ...
    if field_name == "CurrentDirectory":
        return os.getcwd()
    if field_name == "User":
        # Windows sets USERNAME, not USER. Every SigmaHQ rule carrying a `User`
        # selection would otherwise compare against "" and never fire there.
...
def evaluate_rule(rule, command, image):
    ...
    # Each ``*_minus_filters`` variant shares its sibling's branch: the filter
    # half of "selection and not filter" is applied unconditionally by the
    # global suppression below, whatever the condition_type, so the two need
    # the same selection test and nothing more. Written out separately, the
    # copies were byte-identical and free to drift.
    if condition_type in ("single_selection", "selection_minus_filters"):
        sel_key = next(
            (k for k in sel_results if k.startswith("selection")), None
        )
        selection_match = sel_results.get(sel_key, False) if sel_key else False

    elif condition_type in ("all_selections", "all_selections_minus_filters"):
        if sel_results:
            selection_match = all(sel_results.values())
        else:
            selection_match = False

    elif condition_type in ("any_selection", "any_selection_minus_filters"):
...
# Control characters, minus nothing: none of these fields is meant to carry a
# newline, a tab or a terminal escape.
_CONTROL_CHARS = re.compile(r"[\x00-\x1f\x7f-\x9f]+")


def sanitize_rule_text(text, limit):
    """Make third-party rule prose safe to place in a message the model reads.

    ``title``, ``description`` and ``references`` come from YAML in a repo
    ForceField clones and never reviews, and they are copied verbatim into
    ``permissionDecisionReason`` -- which is model context. That is TIER 3 data
    crossing into a TIER 1 position, in the one guard whose whole input is
    third-party. A rule titled with a role tag or an instruction override would
    be read by the agent as part of ForceField's own security message.

    So: collapse control characters and newlines (no field-injection into the
    structured message), neutralize the angle brackets that make a role tag a
    role tag, and cap the length. This is not a claim to have made arbitrary
    prose safe -- it removes the shapes that carry authority.
    """
...
def format_alert(rule):
    ...
    title = sanitize_rule_text(rule.get("title", ""), 200) or "Unknown Rule"
    level = sanitize_rule_text(rule.get("level", ""), 20) or "unknown"
    description = sanitize_rule_text(rule.get("description", ""), 500)
    tags = rule.get("tags", [])
    raw_refs = rule.get("references") or []
    references = [sanitize_rule_text(r, 200) for r in raw_refs
                  if sanitize_rule_text(r, 200)]

    mitre_techniques = [
        sanitize_rule_text(t.replace("attack.", "").upper(), 20)
        for t in tags if isinstance(t, str) and t.startswith("attack.t")
    ]
    tactics = [
...
def main():
    try:
        input_data = json.loads(read_stdin_text(MAX_STDIN_BYTES))
    ...
    if not matched_rules:
        # "No rule matched" was previously indistinguishable from "there are no
        # rules", "the floor emptied the set" and "the engine never ran". The
        # first three are now reported once per session on `session.start`
        # (rules_present / rules_count / severity_floor), which leaves exactly
        # one meaning for silence -- and this record makes even that explicit at
        # debug level.
        log_guard_ran("sigma_engine", context)
        emit({})
        return
